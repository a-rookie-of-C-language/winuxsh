...
#[test]
fn zsh_profile_plan_generates_agent_baseline() {
    let plan = zsh_profile_plan_toml(ZshNativeProfile::Agent);

    assert!(plan.contains("# Generated by winuxsh --zsh-profile-plan agent"));
    assert!(plan.contains("[zsh]"));
    assert!(plan.contains("enabled = true"));
    assert!(plan.contains("auto_apply = false"));
    assert!(plan.contains("import_zshrc = false"));
    assert!(plan.contains("plugins = []"));
    assert!(plan.contains("[zsh.autosuggestions]"));
    assert!(plan.contains("strategy = [\"history\"]"));
    assert!(plan.contains("[zsh.syntax_highlighting]"));
    assert!(plan.contains("highlighters = [\"main\"]"));
...
#[test]
fn zsh_profile_plan_generates_zsh_lite_profile() {
    let plan = zsh_profile_plan_toml(ZshNativeProfile::ZshLite);

    assert!(plan.contains("# Generated by winuxsh --zsh-profile-plan zsh-lite"));
    assert!(plan.contains("auto_apply = true"));
    assert!(plan.contains("import_zshrc = true"));
    assert!(plan.contains("import_oh_my_zsh = true"));
...
#[test]
fn zsh_profile_plan_rejects_unknown_profiles() {
    let err = zsh_profile_plan_toml_for_name("full").unwrap_err();

    assert!(err.to_string().contains("unknown zsh profile"));
}

#[test]
fn zsh_profile_plan_reuses_managed_import_block_mechanics() {
    let temp = unique_temp_dir("winuxsh-zsh-profile-plan-managed");
    std::fs::create_dir_all(&temp).unwrap();
    let config_path = temp.join(".winshrc.toml");
    let plan = zsh_profile_plan_toml(ZshNativeProfile::ZshLite);

    let summary =
        apply_import_plan_to_config_with_backup_suffix(&config_path, &plan, "profile").unwrap();
    ...
    assert!(written.contains("--zsh-profile-plan zsh-lite"));
    assert!(written.contains(ZSH_IMPORT_BLOCK_END));

    let status = inspect_import_config_status(&config_path, &plan).unwrap();
...
#[test]
fn imports_native_direnv_preset_when_omz_plugin_dir_is_missing() {
    let temp = unique_temp_dir("winuxsh-zsh-native-direnv-preset");
    std::fs::create_dir_all(&temp).unwrap();
    std::fs::write(
        temp.join(".zshrc"),
        r#"
...
"#,
    ...
    let direnv = plugin(&report, "direnv");
    assert!(direnv.source_dir.is_none());
    assert_eq!(direnv.import_kind, PluginImportKind::NativeUx);
    assert_eq!(direnv.tier, PluginImportTier::Tier3Native);
    assert!(direnv
        .capabilities
        .iter()
    ...
    assert!(plan.contains("[zsh.native_plugins]"));
    assert!(plan.contains("enabled = false"));
    assert!(plan.contains("presets = [\"direnv\"]"));
...
#[test]
fn imports_native_thefuck_preset_when_omz_plugin_dir_is_missing() {
    let temp = unique_temp_dir("winuxsh-zsh-native-thefuck-preset");
    std::fs::create_dir_all(&temp).unwrap();
    std::fs::write(
        temp.join(".zshrc"),
        r#"
...
"#,
    ...
    let thefuck = plugin(&report, "thefuck");
    assert!(thefuck.source_dir.is_none());
    assert_eq!(thefuck.import_kind, PluginImportKind::NativeUx);
    assert_eq!(thefuck.tier, PluginImportTier::Tier3Native);
    assert!(thefuck
        .capabilities
        .iter()
        .any(|cap| cap == "native_plugins_required"));
    ...
    assert!(plan.contains("[zsh.native_plugins]"));
    assert!(plan.contains("enabled = false"));
    assert!(plan.contains("presets = [\"thefuck\"]"));
...
#[test]
fn imports_native_dotenv_preset_when_omz_plugin_dir_is_missing() {
    let temp = unique_temp_dir("winuxsh-zsh-native-dotenv-preset");
    std::fs::create_dir_all(&temp).unwrap();
    std::fs::write(
        temp.join(".zshrc"),
        r#"
...
"#,
    ...
    let plugin = plugin(&report, "dotenv");
    assert!(plugin.source_dir.is_none());
    assert_eq!(plugin.import_kind, PluginImportKind::NativeUx);
    assert_eq!(plugin.tier, PluginImportTier::Tier3Native);
    assert!(plugin
    ...
    assert!(plan.contains("[zsh.native_plugins]"));
    assert!(plan.contains("enabled = false"));
    assert!(plan.contains("presets = [\"dotenv\"]"));
