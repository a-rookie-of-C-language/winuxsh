...
pub const WINUXSH_PROFILE_GUID: &str = "{7d8b7341-8f4d-4c56-91f0-4ad220d41db1}";
const PROFILE_NAME: &str = "Winuxsh";
...
pub fn install_winuxsh_profile(
    commandline: &Path,
    icon: Option<&Path>,
    set_default: bool,
...
fn upsert_profile(root: &mut Value, commandline: &Path, icon: Option<&Path>, set_default: bool) {
    let profile = winuxsh_profile(commandline, icon);

    {
        ...
        if let Some(existing) = list.iter_mut().find(|candidate| {
            candidate
                ...
                .is_some_and(|guid| guid.eq_ignore_ascii_case(WINUXSH_PROFILE_GUID))
                || candidate
                    .get("name")
                    .and_then(Value::as_str)
    ...
    if set_default || has_invalid_default_profile(root) {
        root.as_object_mut()
            ...
            .insert("defaultProfile".to_string(), json!(WINUXSH_PROFILE_GUID));
    }
}

fn winuxsh_profile(commandline: &Path, icon: Option<&Path>) -> Value {
    let mut profile = json!({
        "guid": WINUXSH_PROFILE_GUID,
        "name": PROFILE_NAME,
...
mod tests {
    ...
    #[test]
    fn creates_settings_with_winuxsh_profile() {
        let temp = tempfile::tempdir().unwrap();
        let settings = temp.path().join("settings.json");

        let summary = install_winuxsh_profile_in_settings(
            Path::new("C:/Users/me/AppData/Local/Programs/Winuxsh/winuxsh.exe"),
            Some(Path::new(
                "C:/Users/me/AppData/Local/Programs/Winuxsh/assets/winuxsh-icon-256.png",
            )),
            false,
            [settings.clone()],
        )
        ...
        assert_eq!(profile["name"], "Winuxsh");
        assert_eq!(profile["guid"], WINUXSH_PROFILE_GUID);
    ...
    fn updates_existing_profile_and_can_set_default() {
        ...
        install_winuxsh_profile_in_settings(
            Path::new("D:/Apps/Winuxsh/winuxsh.exe"),
            None,
            true,
            [settings.clone()],
        )
        ...
        assert_eq!(list[0]["name"], "Winuxsh");
        assert_eq!(list[0]["colorScheme"], "Campbell");
        assert_eq!(root["defaultProfile"], WINUXSH_PROFILE_GUID);
    }
