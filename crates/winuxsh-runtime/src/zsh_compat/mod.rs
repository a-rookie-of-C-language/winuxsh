...
pub enum ZshNativeProfile {
    Agent,
    ZshLite,
}

impl ZshNativeProfile {
    pub fn parse(value: &str) -> anyhow::Result<Self> {
        match value.to_ascii_lowercase().as_str() {
            "agent" => Ok(Self::Agent),
            "zsh-lite" | "zsh_lite" => Ok(Self::ZshLite),
            other => Err(anyhow!(
                "unknown zsh profile '{}'; expected agent or zsh-lite",
                other
            )),
        }
    }
    ...
    pub fn name(self) -> &'static str {
        match self {
            Self::Agent => "agent",
            Self::ZshLite => "zsh-lite",
        }
    }
}

pub fn zsh_profile_plan_toml_for_name(profile: &str) -> anyhow::Result<String> {
    Ok(zsh_profile_plan_toml(ZshNativeProfile::parse(profile)?))
}

pub fn zsh_profile_plan_toml(profile: ZshNativeProfile) -> String {
    let mut out = Vec::new();
    out.push(format!(
        "# Generated by winuxsh --zsh-profile-plan {}",
        profile.name()
    ));
    ...
    match profile {
        ZshNativeProfile::Agent => append_agent_profile_plan(&mut out),
        ZshNativeProfile::ZshLite => append_zsh_lite_profile_plan(&mut out),
    }

    out.join("\n")
}

fn append_agent_profile_plan(out: &mut Vec<String>) {
    out.push("[zsh]".to_string());
    out.push("enabled = true".to_string());
    out.push("auto_apply = false".to_string());
    out.push("import_zshrc = false".to_string());
    out.push("import_oh_my_zsh = false".to_string());
    out.push("compat_level = \"safe\"".to_string());
    out.push("plugins = []".to_string());
    append_safe_ui_profile_sections(out);
    append_disabled_profile_sections(out, false);
}

fn append_zsh_lite_profile_plan(out: &mut Vec<String>) {
    out.push("[zsh]".to_string());
    out.push("enabled = true".to_string());
    out.push("auto_apply = true".to_string());
    out.push("import_zshrc = true".to_string());
    out.push("import_oh_my_zsh = true".to_string());
    out.push("compat_level = \"safe\"".to_string());
    out.push(format!(
        "plugins = {}",
        toml_array(&[
            ...
            "zsh-history-substring-search".to_string(),
        ])
    ));
    append_safe_ui_profile_sections(out);
    append_disabled_profile_sections(out, true);
}

fn append_safe_ui_profile_sections(out: &mut Vec<String>) {
    out.push(String::new());
    out.push("[zsh.autosuggestions]".to_string());
    out.push("enabled = true".to_string());
    out.push("strategy = [\"history\"]".to_string());

    out.push(String::new());
    out.push("[zsh.syntax_highlighting]".to_string());
    out.push("enabled = true".to_string());
    out.push("highlighters = [\"main\"]".to_string());
}

fn append_disabled_profile_sections(out: &mut Vec<String>, enable_widgets: bool) {
    out.push(String::new());
    out.push("[zsh.native_widgets]".to_string());
    out.push(format!("enabled = {}", enable_widgets));
    if enable_widgets {
        out.push(format!(
            "presets = {}",
            toml_array(&[
                "autosuggestions".to_string(),
                "history_substring_search".to_string(),
            ])
        ));
        out.push("import_bindkeys = true".to_string());
    } else {
        out.push("presets = []".to_string());
        out.push("import_bindkeys = false".to_string());
    }
