...
fn user_theme_dir() -> Option<PathBuf> {
    crate::path_utils::shell_home_dir().map(|home| home.join(".niubash").join("themes"))
}
...
mod tests {
    ...
    fn loads_user_theme_from_theme_dir() {
        let dir = unique_temp_dir("niubash-theme-load");
...
"#,
    ...
    fn invalid_user_theme_color_is_ignored() {
        let dir = unique_temp_dir("niubash-theme-invalid");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
...
"#,
    ...
    fn loads_hex_background_and_style_bits_from_theme_file() {
        let dir = unique_temp_dir("niubash-theme-hex");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join("neon.toml"),
            r##"
...
"##,
    ...
    fn invalid_hex_theme_color_is_ignored() {
        let dir = unique_temp_dir("niubash-theme-invalid-hex");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join("broken.toml"),
            r##"
...
"##,
    ...
    fn user_theme_entries_from_dir_lists_valid_safe_themes() {
        let dir = unique_temp_dir("niubash-theme-catalog");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
...
"#,
    ...
    fn unsafe_theme_names_are_ignored() {
        let dir = unique_temp_dir("niubash-theme-unsafe");
        std::fs::create_dir_all(&dir).unwrap();
