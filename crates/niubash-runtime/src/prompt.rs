...
mod tests {
    ...
    fn omits_right_prompt_when_unset() {
        let prompt = NiubashPrompt::new(Some("left> ".to_string()), None, None, "default");
    ...
    fn time_tokens_render_system_local_clock() {
        let prompt =
            NiubashPrompt::new(Some("{time} {time_24}".to_string()), None, None, "default");
    ...
    fn reads_branch_from_git_head() {
        let dir = unique_temp_dir("niubash-prompt-git-head");
    ...
    fn renders_git_prompt_only_inside_git_repo() {
        let dir = unique_temp_dir("niubash-prompt-git-render");
    ...
    fn cwd_token_accepts_shell_style_home_env() {
        ...
        let prompt = NiubashPrompt::new(Some("{cwd}".to_string()), None, None, "default");
        let rendered = prompt.render_prompt_left();
    ...
    fn structured_git_snapshot_is_repainted_with_theme_colours() {
        let mut prompt = NiubashPrompt::new(
