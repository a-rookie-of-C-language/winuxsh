...
[[ -f "$ROOT_REAL/Cargo.toml" ]] || die "Refusing to run outside winuxsh repo: missing Cargo.toml at $ROOT_REAL"
[[ -f "$ROOT_REAL/scripts/run-bash-upstream-with-winuxsh.sh" ]] || die "Refusing to run outside winuxsh repo"
...
for runner in "${RUNNERS[@]}"; do
  ...
  shell_wrapper="$workdir/winuxsh-wrapper"
...
case "\$cwd" in
  "\$allowed"|"\$allowed"/*) ;;
  *)
    echo "Refusing to start winuxsh outside Bash upstream work dir: \$cwd" >&2
    echo "Allowed: \$allowed" >&2
    exit 126
    ;;
esac
...
{
  echo "# Winuxsh Bash Upstream Test Progress"
  echo
  echo "- Total: $TOTAL"
  echo "- Passed: $PASS"
  echo "- Failed: $FAIL"
